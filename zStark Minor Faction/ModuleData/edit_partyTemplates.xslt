<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="MBPartyTemplate[@id='caravan_template_sturgia']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="2" max_value="5" troop="NPCCharacter.stark_cavalry" />
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="MBPartyTemplate[@id='elite_caravan_template_sturgia']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="2" max_value="2" troop="NPCCharacter.stark_heavy_cavalry" />
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="MBPartyTemplate[@id='villager_sturgia_template']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="3" max_value="15" troop="NPCCharacter.stark_swordsman" />
		</xsl:copy>
	</xsl:template>
</xsl:stylesheet>
