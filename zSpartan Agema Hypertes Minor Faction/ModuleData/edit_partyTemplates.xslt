<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="MBPartyTemplate[@id='caravan_template_battania']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="2" max_value="9" troop="NPCCharacter.spartan_heraclid" />
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="MBPartyTemplate[@id='elite_caravan_template_battania']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="5" max_value="15" troop="NPCCharacter.lacedaemon_warrior" />
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="MBPartyTemplate[@id='villager_battania_template']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="3" max_value="10" troop="NPCCharacter.lacedaemon_phalanx" />
		</xsl:copy>
	</xsl:template>
</xsl:stylesheet>
