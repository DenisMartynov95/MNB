<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template match="MBPartyTemplate[@id='caravan_template_battania']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="5" max_value="17" troop="NPCCharacter.celtic_recruit" />
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="MBPartyTemplate[@id='elite_caravan_template_battania']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="5" max_value="8" troop="NPCCharacter.celtic_fighter" />
		</xsl:copy>
	</xsl:template>
	...
	<xsl:template match="MBPartyTemplate[@id='villager_battania_template']/stacks">
		<xsl:copy>
			<xsl:apply-templates select="@* | node()"/>
			<PartyTemplateStack min_value="3" max_value="8" troop="NPCCharacter.celtic_gallowglass" />
		</xsl:copy>
	</xsl:template>
</xsl:stylesheet>
